...
$(function(){
	...
	websocket.onmessage = function(event) {
		...
		if(msg.type == "changetvnewspfrequncy")       //接收滚动频率刷新  
		{
		...
		else if(msg.type == "seltvnewsshow"){        //接收到条目上屏请求
		...
		else if(msg.type == "canceltvnewsshow"){        //接收到条目取消投屏请求
			var showid   = msg.content;			
			if(null==showid || ""==showid)
				return;
		...
		else if(msg.type == "allseltvnewscancel"){      //接收到全部条目取消投屏请求
...
function init () 
{
	intervalTime = parseInt(tvnewsSwitchFrequncy);
	if(null==intervalTime || Number.isNaN(intervalTime))
		intervalTime = 2000;
	else
		intervalTime = intervalTime *1000;
	...
	dp.util.ajaxRequest({
				...
				success : function(data) {
					if (null!=data && null!=data.status && data.status == true ) {
						if(data.newslist!=null && typeof (data.newslist) != "undefined"){
							
							for (var i = 0; i < data.newslist.length; i++) {
								
								if(data.newslist[i].mainHeader.length>30){
			        				data.newslist[i].mainHeader=data.newslist[i].mainHeader.substring(0,30)+"...";
			        			}
							}
							
							var newsListHtml = template('manuscript_script', data);
							$('#news_cont').append(newsListHtml);
							
							var htmltip="未选择投频稿件，系统自动展示" + data.newslist.length + "篇最新电视稿件";
...
function AddShowList(newsinfo){
	...
	dp.util.ajaxRequest({
		type : 'POST',
		url : '/DYBigScreen/DYNewsCommandMobileInterface/tvnewsdetailquery.do',
		data : JSON.stringify(params),
		dataType : 'json',
		contentType : "application/json;charset=UTF-8",
		success : function(data) {
			if (null!=data && null!=data.status && data.status == true ) {
				if(data.newslist!=null && typeof (data.newslist) != "undefined" && data.newslist.length>0){						
						
					if(data.newslist[0].mainHeader.length>30){
        				data.newslist[0].mainHeader=data.newslist[0].mainHeader.substring(0,30)+"...";
        			}
					data.data = data.newslist[0];
					
					if(projectshowlistNum == 0)
						$('#news_cont').html("");
					
					projectshowlistNum	= projectshowlistNum +1;     //增加挑选显示记录
					projectshowlist[newsinfo.id] = 1;
					...
					var htmltip="总共投屏" + projectshowlistNum + "篇电视稿件";
					$('#tipsshow').html(htmltip);
				}
			}
			else {
