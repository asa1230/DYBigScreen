...
function loadSelectApiList() {
	...
	$.ajax({
	        ...
	        async : false,
	        dataType : 'json',
			url : '/DYBigScreen/HZInterfaceSet/getManuScriptList.do?',
	        ...
	        success : function(data) {
            	if (data.commonResponse.success == true) {
            		...
            		for(var m=0;m<rlength;m++){
            			...
            			if(data.records[m].newsTitle.length>20){
            				data.records[m].newsTitle=data.records[m].newsTitle.substring(0,20)+"...";
            			}
            		}
