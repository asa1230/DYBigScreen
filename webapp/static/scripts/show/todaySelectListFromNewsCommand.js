	        ...
	        success : function(data) {
            	if (data.commonResponse.success == true) {
            		...
            		for(var m=0;m<rlength;m++){
            			if(data.records[m].reportsource=="1"){
            				data.records[m].reportFrom="综合频道";
            			}else if(data.records[m].reportsource=="2"){
            				data.records[m].reportFrom="明珠频道";
            			}else if(data.records[m].reportsource=="3"){
            				data.records[m].reportFrom="生活频道";
            			}else if(data.records[m].reportsource=="4"){
            				data.records[m].reportFrom="影视频道";
            			}else if(data.records[m].reportsource=="5"){
            				data.records[m].reportFrom="少儿频道";
            			}else if(data.records[m].reportsource=="6"){
            				data.records[m].reportFrom="移动电视";
            			}else if(data.records[m].reportsource=="7"){
            				data.records[m].reportFrom="房产频道";
            			}else if(data.records[m].reportsource=="8"){
            				data.records[m].reportFrom="杭州之声";
            			}else if(data.records[m].reportsource=="9"){
            				data.records[m].reportFrom="西湖之声";
            			}else if(data.records[m].reportsource=="10"){
            				data.records[m].reportFrom="交通经济广播";
            			}else if(data.records[m].reportsource=="11"){
            				data.records[m].reportFrom="影视周报";
            			}else if(data.records[m].reportsource==""){
            				data.records[m].reportFrom="";
            			}
            			
            			
            			//对于过长的title 进行截取 长度超过20的减为20
            			if(data.records[m].newsTaskTitle.length>16){
            				data.records[m].newsTaskTitle=data.records[m].newsTaskTitle.substring(0,16)+"...";
            			}
            			if(data.records[m].taskStateValue==2){
            				data.records[m].taskState='选题终止';
            			}else if(data.records[m].taskStateValue==0){
            				data.records[m].taskState='进行中';
            			}else if(data.records[m].taskStateValue==104){
            				data.records[m].taskState='未指派';
            			}else if(data.records[m].taskStateValue==1){
            				data.records[m].taskState='已完成';
            			}else if(data.records[m].taskStateValue==8){
