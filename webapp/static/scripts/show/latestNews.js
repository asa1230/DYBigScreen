...
function loadSelectApiList() {
	...
	$.ajax({
	        ...
	        async : false,
	        dataType : 'json',
			url : '/DYBigScreen/HZInterfaceSet/getLatestNews.do',
	        ...
	        success : function(data) {
            	if (data.commonResponse.success == true) {
            		...
            		for(var m=0;m<rlength;m++){
            			...
            			if(data.records[m].title.length>21){
            				data.records[m].title=data.records[m].title.substring(0,21)+"...";
            			}
            			data.records[m].publish_time=data.records[m].publish_time.substring(5,16);
            		}
