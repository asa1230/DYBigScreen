        ...
        initControl:function(){
        	...
        	websocket.onmessage = function(event){
        		...
        		if(msg.type == "mapMove"){
        			...
        			$("#li_"+msg.activeId).attr("class","tab-current active");
        		...
        		else if(msg.type == "mapmarkclick"){      //zhy add for currentview mapmark click
        			if(msg.iconType=="1"){        				
        				c.msgaddClickHandler(msg.resid,msg.tabType,msg.showstate);
        			}else{
        				c.msgaddClickHandler2(msg.resid,msg.tabType,msg.showstate);
        			}
        		}
        	}
...
//					break;
		...
		//收到控制端点击地图标记点消息1
		msgaddClickHandler : function(id, type, bShow) {
		...
		msgaddClickHandler2: function(id, type, bShow) {
		...
		*/
		 ...
		 * 根据查询条件重置列表数据缓存
		 */
		resetCacheByQuery : function(type,bAsync) {
			// 获取type列表下的筛选条件
			...
			c.initCacheDate(type, state, searchContent,bAsync);
		},
		/**
		 * 加载当前列表之外的其他地图图标
		 */
