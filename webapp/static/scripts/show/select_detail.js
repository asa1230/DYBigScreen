	...
	ajaxRequest({
		...
		success : function(data) {			
			...
			if (data.commonResponse.success == true) {
				...
				var resources = data.resourceList;
				var resourcesArr = [];
				if(resources){
				...
				data.taskModelList = taskArr;
				data.resourceList = resourcesArr;
				console.info(JSON.stringify(data));
				...
				console.log(data.resourceList.length);
				...
				//	var zyNum = data.resourceList.length;
				//	$("#zyNum").html(zyNum);
				//}else{
				//	var html_4 = template('select4-list-item', data);
				//	document.getElementById('select-4').innerHTML = html_4;
