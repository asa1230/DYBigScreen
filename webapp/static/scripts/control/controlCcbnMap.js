	...
	return {
		...
		initEvent : function() {
			...
			$(":checkbox").bind('change', function() {
				if (this.checked) {
					// 加载列表之外的其他地图图标
					c.loadOtherIcon(activeId);
					var msg = '{"type":"mapPointChecked","activeId":"'+activeId+'"}';
		    		send(msg);
				} else {
					// 删除列表之外的其他地图图标
					$("img[class^='mapicon2_']").parent().remove();
					var msg = '{"type":"mapPointNone","activeId":"'+activeId+'"}';
		    		send(msg);
				}
			});
					...
					jump : function(e, first) {
						if (!first) {
							...
							c.addMouseToTabList(type);
          					...
          					var msg = '{"type":"pageJump","url":"'+""+'","currPage":"'+starPage+'","totalPage":"'+data.page_info.total_page+'","tabType":"'+type+'"}';
          					send(msg);
						}
		...
		showTabMapPoint : function(type, currPage) {
			...
			if (data != null && data.records.length > 0) {
				for (var i = 0; i < data.records.length; i++) {
					...
					c.addClickHandler(marker, records.id, type,'1');
		...
		focusPoint : function(Longitude, Lantitude) {
			if (Longitude != null && Longitude.length > 0 && Lantitude != null
				...
				var msg = '{"type":"mapMove","MAP":[{"LNG":"'+Longitude+'"},{"LAT":"'+Lantitude+'"},{"ZOOM":"17"},{"TYPE":"地图"}]}';
        		send(msg);
			}
		},
		/**
		 * 给marker添加点击事件
		 */
		addClickHandler : function(marker, id, type,iconType) {
			if(type=='4' && iconType=='1'){
	        	marker.addEventListener("click",
						function(e){
	        					//清除其他图标的点击效果
	        					$("img[class^='mapicon1']").prop("src","../../ccbn/images/blue.png");
	        					$("div[id^='label_']").hide();
	        					$(".mapicon1_"+id).prop("src","../../ccbn/images/red.png");
	        					$("#label_"+type+"_"+id).show();
	        					c.toDetail(id);
	        					var msg = '{"type":"userClick","userId":"'+id+'","tabType":"'+type+'"}';
	        		    		send(msg);
						}
	        	);
        	}else if(!IsPC()){
        		...
        		marker.addEventListener("click",
						function(e){
        					if(iconType=='1'){
        						if($("#label_"+type+"_"+id).is(":hidden")){
        							$("img[class^='mapicon1']").prop("src","../../ccbn/images/blue.png");
        							$("div[id^='label_']").hide();
        							$("div[id^='label2_']").hide();
        							$(".mapicon1_"+id).prop("src","../../ccbn/images/yellow.png");
        							$("#label_"+type+"_"+id).show();
        							
        							var msg = '{"type":"iconShow","iconId":"'+id+'","tabType":"'+type+'"}';
    	        		    		send(msg);
        						}else{
        							$(".mapicon1_"+id).prop("src","../../ccbn/images/blue.png");
        							$("#label_"+type+"_"+id).hide();
        							var msg = '{"type":"iconHide","iconId":"'+id+'","tabType":"'+type+'"}';
        							send(msg);
        						}
        					}else{
        						if($("#label2_"+type+"_"+id).is(":hidden")){
        							//清除其他图标的点击效果
        							...
        							var msg = '{"type":"iconShow2","iconId":"'+id+'","tabType":"'+type+'"}';
        							send(msg);
        						}else{
        							$("#label2_"+type+"_"+id).hide();
        							var msg = '{"type":"iconHide2","iconId":"'+id+'","tabType":"'+type+'"}';
        							send(msg);
        						}
        					}
						}
		...
		addMouseHandler : function(marker, id, type, iconType) {
			if(IsPC()){
        		marker.addEventListener("mouseover",
        				function(e){
        			if(iconType=='1'){
        				$(".mapicon1_"+id).each(function(){
        					var src =  $(this).prop("src");
        					if(src.indexOf("red")<=0){
        						$(this).prop("src","../../ccbn/images/yellow.png");
        						$("#label_"+type+"_"+id).show();
        						var msg = '{"type":"labelShow","labelId":"'+id+'","iconType":"'+iconType+'","tabType":"'+type+'"}';
        						send(msg);
        					}
        				});
        			}else{
        			...
        			if(iconType=='1'){
        				$(".mapicon1_"+id).each(function(){
        					...
        					if(src.indexOf("red")<=0){
        						$(this).prop("src","../../ccbn/images/blue.png");
        						$("#label_"+type+"_"+id).hide();
        						var msg = '{"type":"labelHide","labelId":"'+id+'","iconType":"'+iconType+'","tabType":"'+type+'"}';
        						send(msg);
        					}
        				});
        			}else{
		...
		addMouseToTabList : function(type) {
			$('#section-iconbox-' + type).find(".data-cont").bind(
					...
					function() {
						...
						var msg = '{"type":"listOver","classStr":"'+classStr+'"}';
						send(msg);
					});
			$('#section-iconbox-' + type).find(".data-cont").bind(
					'mouseout',
					function() {
						...
						var msg = '{"type":"listOut","classStr":"'+classStr+'"}';
						send(msg);
					});
		},
		/**
			...
			for (var i = 1; i <= 5; i++) {
				...
				if (otherData != null && otherData.records.length > 0) {
					...
					for (var j = 0; j < otherData.records.length; j++) {
						...
						c.addClickHandler(marker,records.id,type,'2');
						map.addOverlay(marker);
					}
					// 设置marker为透明
